...
def create_raw_transactions(max_num_tx, enrolled_users):
    ...
    for i in range(num_tx):
        ...
        print("-Attempt by user "+str(id_user));
...
#        dt = datetime.now(); #actual date and time
        ...
        raw_transactions.append([m,signature]);
...
#Create block
def create_block(aux, w, difficulty_coefficient, max_num_tx, block_number, raw_transactions, seed_values, user_identities):

    difficulty, nonce = classical_pow(max_num_tx, w, difficulty_coefficient, aux, block_number, raw_transactions, seed_values, user_identities);
    ...
    for i in range(len(raw_transactions)):
        ...
        m = raw_transactions[i][0];
        signature = raw_transactions[i][1];
        mined_tx = {'Tx#':i, 'identity':user_identities[i], 'timestamp':m, 'r':hex(signature[0]), 's':hex(signature[1]), 'v':signature[2], 'PoW':seed_values[i]};
...
def classical_pow(max_num_tx, w, difficulty_coefficient, aux, block_number, raw_transactions, seed_values, user_identities):
