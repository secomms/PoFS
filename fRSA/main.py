...
from RSA_utils import RSA_user, clearing
...
enrolled_users = [RSA_user(w) for i in range(N)];
...
for block_number in range(1,num_blocks+1):
    ...
    for i in range(len(raw_transactions)):
        ...
        user_id = raw_transactions[i][0]; #identity of user that prepared the transaction
        m = raw_transactions[i][1]; #message to be signed in i-th transaction in the block
        signature = raw_transactions[i][2]; #fuzzy signature in i-th transaction in the block

        #Reading public key from file
        this_public_key = public_keys_file[user_id];
        ...
        while flag_cleared == 0:
            num_attempts += 1;
            flag_cleared, test_seed, delta_e = clearing(m, w, signature, X, aux, this_public_key); #clearing attempt
        ...
        print("- User #"+str(user_id)+", Golden Delta_e = "+str(delta_e));
    ...
    aux, json_block = create_block(aux, w, difficulty_coefficient, max_num_tx, block_number, raw_transactions, seed_values);
